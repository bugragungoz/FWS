...
<#
.SYNOPSIS
    ANSYS Electronics Suite 2024 R1 Internet Access Blocker - Localhost License Exception

.DESCRIPTION
    Blocks all Inbound and Outbound traffic to/from any IP except 127.0.0.1 for ANSYS
    exe via Windows Firewall. Preserves local license server (1055@localhost).
    RemoteAddress blocks 0.0.0.0-127.0.0.0 and 127.0.0.2-255.255.255.255 - only 127.0.0.1
    remains accessible for license verification.

.NOTES
    Name:           ANSYS Internet Access Blocker
    Author:         Bugra
    Version:        1.1.0
    Created:        2025
    Target:         ANSYS Electronics Suite 2024 R1
    License:        Preserves 127.0.0.1 (1055@localhost)
...
$script:Config = @{
    LogDirectory    = "$PSScriptRoot\ANSYSBlocker_Logs"
    BackupDirectory = "$PSScriptRoot\ANSYSBlocker_Backups"
    LogFile         = ""
    BackupFile      = ""
    ReportFile      = ""
    SessionID       = (Get-Date -Format "yyyyMMdd_HHmmss")
    DryRun          = $false
    RulePrefix      = "App Internet Blocker - ANSYS"
    RuleGroup       = "ANSYS Internet Blocker"
}
...
# Blocks all IPs except 127.0.0.1 - preserves local license server (1055@localhost)
...
function Show-Banner {
    Write-Host ""
    Write-Host "  ANSYS Internet Blocker v1.1.0 | Session: $($script:Config.SessionID)" -ForegroundColor Cyan
    Write-Host "  Rule Group: $($script:Config.RuleGroup)" -ForegroundColor Gray
    Write-Host "  Target: ANSYS Electronics Suite 2024 R1" -ForegroundColor Gray
    Write-Host "  [*] Blocks ALL IPs except 127.0.0.1 - Inbound + Outbound (exe only)" -ForegroundColor Yellow
    Write-Host "  [*] Localhost (127.0.0.1:1055) preserved for local license server" -ForegroundColor Green
    Write-Host ""
}
...
function Initialize-Environment {
    try {
        ...
        $script:Config.LogFile = Join-Path $script:Config.LogDirectory "ANSYSBlocker_$($script:Config.SessionID).log"
        $script:Config.BackupFile = Join-Path $script:Config.BackupDirectory "FirewallRules_$($script:Config.SessionID).json"
        $script:Config.ReportFile = Join-Path $script:Config.LogDirectory "ANSYSBlocker_Report_$($script:Config.SessionID).txt"
...
function Backup-FirewallRules {
    ...
    try {
        ...
        if ($existingRules) {
            ...
            Write-Host "  [INFO] No existing ANSYS blocker rules found" -ForegroundColor Cyan
...
function Get-ANSYSScanDirectories {
    $directories = @()
    ...
    $primaryPaths = @(
        ...
        "$env:LOCALAPPDATA\Ansys",
        "$env:LOCALAPPDATA\ANSYS Inc",
        "$env:APPDATA\Ansys",
        "$env:APPDATA\ANSYS Inc",
    ...
    $searchKeywords = @("ANSYS", "Ansys", "AnsysEM", "HFSS", "Maxwell", "Q3D")
    $basePaths = @(
        "C:\Program Files",
        "C:\Program Files (x86)",
        "C:\ProgramData",
...
function New-ANSYSFirewallRule {
    param(
        [string]$DisplayName,
        [string]$FilePath,
...
function Remove-ANSYSFirewallRules {
...
function Show-MainMenu {
    ...
    Write-Host "================================================================================" -ForegroundColor Cyan
    Write-Host ""
    Write-Host "  [1] BLOCK MODE      - Inbound + Outbound, block all except 127.0.0.1 (exe only)" -ForegroundColor Green
...
function Show-BlockModeDisclaimer {
    Write-Host ""
    Write-Host "  BLOCK MODE: Inbound + Outbound - blocks all IPs except 127.0.0.1" -ForegroundColor Yellow
    Write-Host "  Preserves local license server (1055@localhost). Exe + dll." -ForegroundColor Gray
    Write-Host ""
}
...
function Show-UnblockDisclaimer {
    Write-Host ""
    Write-Host "  UNBLOCK: Remove all ANSYS blocker rules, restore full internet access." -ForegroundColor Yellow
    Write-Host ""
}
...
function Show-DisclaimerAndHelp {
    ...
    Write-Host ""
    Write-Host "================================================================================" -ForegroundColor Cyan
    Write-Host "             ANSYS BLOCKER - DISCLAIMER & HELP DOCUMENTATION                     " -ForegroundColor Cyan
    Write-Host "================================================================================" -ForegroundColor Cyan
    Write-Host ""
    ...
    Write-Host "  - Inbound + Outbound firewall rules for ANSYS exe" -ForegroundColor White
    Write-Host "  - Blocks ALL IPs except 127.0.0.1 (0.0.0.0-127.0.0.0, 127.0.0.2-255.255.255.255)" -ForegroundColor White
    Write-Host "  - PRESERVES 127.0.0.1 (local license server 1055@localhost)" -ForegroundColor Green
    Write-Host ""
    Write-Host "MANUAL CLEANUP - Remove all ANSYS blocker rules:" -ForegroundColor Yellow
    Write-Host "  Get-NetFirewallRule -DisplayName 'App Internet Blocker - ANSYS*' | Remove-NetFirewallRule" -ForegroundColor Gray
    Write-Host ""
...
function Invoke-UnblockMode {
    ...
    try {
        $removed = Remove-ANSYSFirewallRules
        if ($removed -gt 0) {
    ...
    Write-Host ""
    Write-Host "  ANSYS internet access has been restored." -ForegroundColor Green
    Write-Host ""
}
...
function Invoke-ScanAnsysDirectories {
    ...
    $directories = Get-ANSYSScanDirectories

    if ($directories.Count -eq 0) {
        Write-Host "  [WARNING] No ANSYS directories found in standard locations" -ForegroundColor Yellow
    ...
    $files = Get-ANSYSFiles -Directories $directories
    if ($files.Count -eq 0) {
    ...
    for ($i = 0; $i -lt $fileArray.Count; $i++) {
        ...
        if (-not $script:BlockedFilesHashSet.ContainsKey($file.FullName)) {
            ...
            $outboundOk = New-ANSYSFirewallRule -DisplayName $displayName -FilePath $file.FullName -Direction Outbound
            $inboundOk = New-ANSYSFirewallRule -DisplayName $displayName -FilePath $file.FullName -Direction Inbound
...
function New-ExecutionReport {
    ...
    $report = @"
================================================================================
                    ANSYS BLOCKER EXECUTION REPORT
================================================================================
...
Block Mode:       Inbound + Outbound (exe only), 127.0.0.1 preserved
