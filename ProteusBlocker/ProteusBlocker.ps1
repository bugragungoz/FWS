...
<#
.SYNOPSIS
    Proteus Internet Access Blocker - Comprehensive Network Blocking Tool
    
.DESCRIPTION
    Advanced PowerShell script to block internet access for all Proteus applications
    and services through Windows Firewall rules, hosts file modifications, and IP blocking.
    
.NOTES
    Name:           Proteus Internet Access Blocker
    Author:         Bugra
...
$script:Config = @{
    LogDirectory     = "$PSScriptRoot\ProteusBlocker_Logs"
    BackupDirectory  = "$PSScriptRoot\ProteusBlocker_Backups"
    LogFile          = ""
    BackupFile       = ""
    ReportFile       = ""
    SessionID        = (Get-Date -Format "yyyyMMdd_HHmmss")
    DryRun           = $false
    RulePrefix       = "ProteusBlocker"
    RuleGroup        = "ProteusBlocker"
}
...
function Show-Banner {
    ...
    Write-Host "     ######  ##     ##  #######  ##     ## ########" -ForegroundColor Cyan
    Write-Host "                                                                                " -ForegroundColor Cyan
    Write-Host "           Proteus Internet Access Blocker v1.0.0                               " -ForegroundColor Cyan
    Write-Host "                                                                                " -ForegroundColor Cyan
    Write-Host "================================================================================" -ForegroundColor Cyan
    Write-Host ""
...
function Initialize-Environment {
    try {
        ...
        $script:Config.LogFile = Join-Path $script:Config.LogDirectory "ProteusBlocker_$($script:Config.SessionID).log"
        $script:Config.BackupFile = Join-Path $script:Config.BackupDirectory "FirewallRules_$($script:Config.SessionID).json"
        $script:Config.ReportFile = Join-Path $script:Config.LogDirectory "ProteusBlocker_Report_$($script:Config.SessionID).txt"
...
function Show-BlockModeDisclaimer {
    ...
    Write-Host "     - Block all Proteus .exe files" -ForegroundColor White
    ...
    Write-Host "     - Add Proteus/Labcenter domain entries pointing to 0.0.0.0" -ForegroundColor White
    ...
    Write-Host "     - Known Labcenter Electronics IP ranges" -ForegroundColor White
    ...
    Write-Host "  - Blocking may violate Labcenter's Terms of Service" -ForegroundColor Yellow
    Write-Host ""
    Write-Host "================================================================================" -ForegroundColor Yellow
    Write-Host ""
}
...
function Show-DryRunDisclaimer {
    ...
    Write-Host "  - Scan all Proteus directories and files" -ForegroundColor White
    Write-Host "  - Detect Proteus services on your system" -ForegroundColor White
    ...
    Write-Host "  - Verify Proteus installation paths" -ForegroundColor White
...
function Show-UnblockDisclaimer {
    ...
    Write-Host "  - Restore Proteus's internet access" -ForegroundColor White
    Write-Host ""
    Write-Host "  IMPORTANT:" -ForegroundColor Red
    Write-Host "  - This will allow Proteus to connect to the internet again" -ForegroundColor Yellow
...
function Show-DisclaimerAndHelp {
    ...
    Write-Host ""
    Write-Host "================================================================================" -ForegroundColor Cyan
    Write-Host "             PROTEUS BLOCKER - DISCLAIMER & HELP DOCUMENTATION                  " -ForegroundColor Cyan
    Write-Host "================================================================================" -ForegroundColor Cyan
    Write-Host ""
    ...
    Write-Host "     - Creates Windows Firewall rules for Proteus executables" -ForegroundColor White
    ...
    Write-Host "     - Scans common Proteus installation directories" -ForegroundColor White
    Write-Host ""
    ...
    Write-Host "     - Adds Labcenter domain entries (license, update, telemetry)" -ForegroundColor White
    ...
    Write-Host "     - Blocks known Labcenter Electronics IP subnets" -ForegroundColor White
    ...
    Write-Host "     - Scans for Proteus-related Windows services" -ForegroundColor White
    ...
    Write-Host ""
    Write-Host "  Problem: Proteus directory not found" -ForegroundColor Yellow
    Write-Host "  Solution:" -ForegroundColor Cyan
    Write-Host "    - Script will prompt for custom directory" -ForegroundColor White
    Write-Host "    - Enter path like: C:\Program Files\Labcenter Electronics\Proteus 8 Professional" -ForegroundColor White
    Write-Host ""
    Write-Host "  Problem: Proteus still connects to internet" -ForegroundColor Yellow
    Write-Host "  Solution:" -ForegroundColor Cyan
    Write-Host "    - Verify Windows Firewall is enabled" -ForegroundColor White
    Write-Host "    - Check rules exist: Get-NetFirewallRule -DisplayName 'ProteusBlocker*'" -ForegroundColor White
    ...
    Write-Host "-------------------------------------------------------------------------------" -ForegroundColor Gray
    Write-Host ""
    Write-Host "  Remove all Proteus blocker rules:" -ForegroundColor Yellow
    Write-Host "    Get-NetFirewallRule -DisplayName 'ProteusBlocker*' | Remove-NetFirewallRule" -ForegroundColor White
    Write-Host ""
    Write-Host "  View current rules:" -ForegroundColor Yellow
    Write-Host "    Get-NetFirewallRule -DisplayName 'ProteusBlocker*' | Select DisplayName, Enabled" -ForegroundColor White
    Write-Host ""
...
function Invoke-UnblockMode {
    ...
    Write-Host "================================================================================" -ForegroundColor Green
    Write-Host ""
    Write-Host "  Proteus internet access has been restored." -ForegroundColor Green
    Write-Host ""
}
...
function Process-ProteusDirectory {
    ...
    $files = Get-ProteusFiles -Path $Path
    
    if ($files.Count -eq 0) {
        Write-Host "  [WARNING] No Proteus files found in this directory" -ForegroundColor Yellow
        return 0
    }
    ...
    for ($i = 0; $i -lt $fileArray.Count; $i++) {
        ...
        Write-Progress -Activity "Processing Proteus Files" -Status "$percentage% Complete" -PercentComplete $percentage -CurrentOperation $file.Name
    ...
    Write-Progress -Activity "Processing Proteus Files" -Completed
...
function Process-SystemLocations {
    ...
    # Find all Proteus installations
    $baseProteusPath = "C:\Program Files\Labcenter Electronics"
    $locations = @()
    
    if (Test-Path $baseProteusPath) {
        $proteusVersions = Get-ChildItem -Path $baseProteusPath -Directory -Filter "Proteus*" -ErrorAction SilentlyContinue
        foreach ($version in $proteusVersions) {
            $locations += $version.FullName
        }
    }
    
    # Add common Proteus locations
    $locations += @(
        "C:\Program Files (x86)\Labcenter Electronics",
        "C:\ProgramData\Labcenter Electronics",
        "$env:LOCALAPPDATA\Labcenter Electronics",
        "$env:APPDATA\Labcenter Electronics"
    )
    
    # Check for Proteus in alternate locations
    $altPaths = @(
        "C:\Proteus",
        "C:\Program Files\Proteus",
        "C:\Program Files (x86)\Proteus"
    )
    
    foreach ($altPath in $altPaths) {
        if (Test-Path $altPath) {
            $proteusVersions = Get-ChildItem -Path $altPath -Directory -Filter "Proteus*" -ErrorAction SilentlyContinue
            foreach ($version in $proteusVersions) {
                $locations += $version.FullName
            }
        }
    ...
    if ($totalProcessed -eq 0) {
        Write-Host ""
        Write-Host "  [WARNING] No Proteus files found in common locations" -ForegroundColor Yellow
        Write-Host "  [INFO] Would you like to specify a custom Proteus directory? (yes/no): " -ForegroundColor Cyan -NoNewline
        $response = Read-Host
        
        if ($response -match '^(yes|y)$') {
            Write-Host "  Enter custom Proteus directory path: " -ForegroundColor Cyan -NoNewline
...
function Process-SpecificLocations {
    ...
    # Find all Proteus versions and add specific paths
    $baseProteusPath = "C:\Program Files\Labcenter Electronics"
    $specificPaths = @()
    ...
    if (Test-Path $baseProteusPath) {
        ...
        foreach ($version in $proteusVersions) {
            ...
            $specificPaths += "$($version.FullName)\LIBRARY"
            $specificPaths += "$($version.FullName)\MODELS"
        }
    }
...
function Block-ProteusDomains {
    Write-Host ""
    Write-Host "[Step 6] Blocking Proteus/Labcenter domains via hosts file..." -ForegroundColor Cyan
    ...
    $domains = @(
        ...
        # License & Activation
        "license.labcenter.com",
        "licensing.labcenter.com",
        "activate.labcenter.com",
        "activation.labcenter.com",
        "licensing-services.labcenter.com",
        "lic.labcenter.com",
        
        # Update & Download
        "update.labcenter.com",
        "updates.labcenter.com",
        "download.labcenter.com",
        "downloads.labcenter.com",
        "updatecheck.labcenter.com",
        "autoupdate.labcenter.com",
        
        # Analytics & Telemetry
        "analytics.labcenter.com",
        "telemetry.labcenter.com",
        "metrics.labcenter.com",
        "stats.labcenter.com",
        "diagnostic.labcenter.com",
        "diagnostics.labcenter.com",
        
        # Account & Auth
        "login.labcenter.com",
        "account.labcenter.com",
        "accounts.labcenter.com",
        "auth.labcenter.com",
        "authentication.labcenter.com",
        "myaccount.labcenter.com",
        
        # Support & Documentation
        "support.labcenter.com",
        "help.labcenter.com",
        "docs.labcenter.com",
        "documentation.labcenter.com",
        
        # API & Services
        "api.labcenter.com",
        "services.labcenter.com",
        "webservices.labcenter.com",
        "webservice.labcenter.com",
        
        # CDN & Assets
        "cdn.labcenter.com",
        "assets.labcenter.com",
        "static.labcenter.com",
        "media.labcenter.com",
        
        # Cloud Services
        "cloud.labcenter.com",
        "online.labcenter.com",
        
        # Additional Services
        "feedback.labcenter.com",
        "survey.labcenter.com",
        "forum.labcenter.com",
        "forums.labcenter.com",
    ...
    try {
        ...
        if ($newEntries.Count -gt 0) {
            $newEntries = @("", "# Proteus Blocker Entries - Added $(Get-Date)") + $newEntries
...
function Block-ProteusIPRanges {
    Write-Host ""
    Write-Host "[Step 7] Blocking Labcenter Electronics IP ranges..." -ForegroundColor Cyan
    
    $ipRanges = @(
        @{ Range = "195.8.215.0/24"; Description = "Labcenter Electronics UK - Primary" },
        @{ Range = "213.249.225.0/24"; Description = "Labcenter Electronics UK - Secondary" },
        @{ Range = "82.132.0.0/16"; Description = "Labcenter Electronics - Cloud Services" }
    )
...
function Check-ProteusServices {
    ...
    try {
        $services = Get-Service | Where-Object { $_.DisplayName -like "*Proteus*" -or $_.DisplayName -like "*Labcenter*" }
        
        if ($services) {
            Write-Host "  [INFO] Found $($services.Count) Proteus-related services:" -ForegroundColor Yellow
            ...
            Write-Host "  [INFO] No Proteus services detected" -ForegroundColor Cyan
            Write-Log "No Proteus services found" -Level INFO
        }
...
function Generate-Report {
    ...
    $report = @"
================================================================================
                    PROTEUS BLOCKER EXECUTION REPORT
================================================================================
...
try {
    ...
    switch ($mode) {
        "1" {
            ...
            Write-Host "================================================================================" -ForegroundColor Green
            Write-Host ""
            Write-Host "  Proteus internet access has been blocked successfully!" -ForegroundColor Green
            Write-Host ""
        }
        "2" {
