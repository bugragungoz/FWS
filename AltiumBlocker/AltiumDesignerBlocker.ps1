...
<#
.SYNOPSIS
    Altium Designer Internet Access Blocker - Selective Network Blocking Tool
    
.DESCRIPTION
    Advanced PowerShell script to block internet access for Altium Designer while
    preserving Component Search, Manufacturer Part Search, and Login functionality.
    
.NOTES
    Name:           Altium Designer Internet Access Blocker
    Author:         Bugra
...
.IMPORTANT
    ...
    - Online component databases (Octopart, PartQuest, etc.)
    - User login and authentication
    - Library access and downloads
...
$script:Config = @{
    LogDirectory     = "$PSScriptRoot\AltiumBlocker_Logs"
    BackupDirectory  = "$PSScriptRoot\AltiumBlocker_Backups"
    LogFile          = ""
    BackupFile       = ""
    ReportFile       = ""
    SessionID        = (Get-Date -Format "yyyyMMdd_HHmmss")
    DryRun           = $false
    RulePrefix       = "AltiumBlocker"
    RuleGroup        = "AltiumBlocker"
}
...
# CRITICAL: Files/components that MUST remain unblocked for Component Search
$script:AllowedComponents = @(
    # Component Search & Part Search
    "ComponentSearch", "Octopart", "PartQuest", "AltiumLive", "Portal",
    "Component", "Library", "LibraryLoader", "VaultQuery", "Manufacturer",
    "PartSearch", "MPSe", "Supplier", "PartChoose", "ActiveBOM", "Search",
    "Query", "Nexus", "Cloud", "Concord", "DataManager", "ContentStore",
    "Provider", "PCBLibrary", "SchLibrary", "IntLibrary",
    
    # Authentication & Login
    "Login", "Auth", "Authentication", "Session", "Identity", "Token",
    "OAuth", "SSO", "Account", "User",
    
    # Main executables (MUST work)
    "X2.exe", "DXP.exe", "Altium Designer.exe", "Altium.exe",
    "Designer.exe", "PCB.exe", "Schematic.exe",
    
    # Online databases
    "Database", "DB", "WebService", "API", "REST", "HTTP"
)
...
function Show-Banner {
    ...
    Write-Host "     ######  ##     ##  #######  ##     ## ########" -ForegroundColor Cyan
    Write-Host "                                                                                " -ForegroundColor Cyan
    Write-Host "         Altium Designer Internet Access Blocker v1.0.0                        " -ForegroundColor Cyan
    Write-Host "                                                                                " -ForegroundColor Cyan
    Write-Host "================================================================================" -ForegroundColor Cyan
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "  [!] SPECIAL MODE: Component Search & Login PRESERVED" -ForegroundColor Green
...
function Initialize-Environment {
    try {
        ...
        $script:Config.LogFile = Join-Path $script:Config.LogDirectory "AltiumBlocker_$($script:Config.SessionID).log"
        $script:Config.BackupFile = Join-Path $script:Config.BackupDirectory "FirewallRules_$($script:Config.SessionID).json"
        $script:Config.ReportFile = Join-Path $script:Config.LogDirectory "AltiumBlocker_Report_$($script:Config.SessionID).txt"
...
function Show-MainMenu {
    ...
    Write-Host "================================================================================" -ForegroundColor Cyan
    Write-Host ""
    Write-Host "  [1] BLOCK MODE      - Selective blocking (preserves Component Search)" -ForegroundColor Green
...
function Show-BlockModeDisclaimer {
    ...
    Write-Host "     - PRESERVE Component Search functionality" -ForegroundColor Green
    Write-Host "     - PRESERVE Manufacturer Part Search" -ForegroundColor Green
    Write-Host "     - PRESERVE Login and authentication" -ForegroundColor Green
    ...
    Write-Host "     - PRESERVE component database access" -ForegroundColor Green
    Write-Host "     - PRESERVE login servers" -ForegroundColor Green
    Write-Host ""
    ...
    Write-Host "     - Component databases remain accessible" -ForegroundColor Green
    Write-Host ""
    ...
    Write-Host "  - Blocking may violate Altium's Terms of Service" -ForegroundColor Yellow
    Write-Host ""
    Write-Host "================================================================================" -ForegroundColor Yellow
    Write-Host ""
}
...
function Show-DryRunDisclaimer {
    ...
    Write-Host "  - Scan all Altium Designer directories and files" -ForegroundColor White
    ...
    Write-Host "  - Verify Component Search files are preserved" -ForegroundColor White
...
function Show-UnblockDisclaimer {
    ...
    Write-Host "  - Restore full Altium Designer internet access" -ForegroundColor White
    Write-Host ""
    Write-Host "  IMPORTANT:" -ForegroundColor Red
    Write-Host "  - This will allow ALL Altium components to connect to internet" -ForegroundColor Yellow
...
function Show-DisclaimerAndHelp {
    ...
    Write-Host ""
    Write-Host "================================================================================" -ForegroundColor Cyan
    Write-Host "           ALTIUM DESIGNER BLOCKER - DISCLAIMER & HELP DOCUMENTATION            " -ForegroundColor Cyan
    Write-Host "================================================================================" -ForegroundColor Cyan
    Write-Host ""
    ...
    Write-Host "  - Component Search functionality" -ForegroundColor White
    Write-Host "  - Manufacturer Part Search" -ForegroundColor White
    Write-Host "  - Octopart integration" -ForegroundColor White
    Write-Host "  - PartQuest database access" -ForegroundColor White
    Write-Host "  - AltiumLive component downloads" -ForegroundColor White
    Write-Host "  - User login and authentication" -ForegroundColor White
    Write-Host "  - Library downloads and updates" -ForegroundColor White
    Write-Host "  - Online component databases" -ForegroundColor White
    Write-Host ""
    Write-Host "  BLOCKED:" -ForegroundColor Red
    Write-Host "  - License validation servers" -ForegroundColor White
    ...
    Write-Host "-------------------------------------------------------------------------------" -ForegroundColor Gray
    Write-Host ""
    Write-Host "  Problem: Component Search not working" -ForegroundColor Yellow
    Write-Host "  Solution:" -ForegroundColor Cyan
    Write-Host "    - This script should preserve Component Search" -ForegroundColor White
    ...
    Write-Host ""
    Write-Host "  Problem: Cannot login to Altium account" -ForegroundColor Yellow
    Write-Host "  Solution:" -ForegroundColor Cyan
    Write-Host "    - Login functionality should be preserved" -ForegroundColor White
...
function Invoke-UnblockMode {
    ...
    Write-Host "================================================================================" -ForegroundColor Green
    Write-Host ""
    Write-Host "  Altium Designer full internet access has been restored." -ForegroundColor Green
    Write-Host ""
}
...
function Test-ShouldBlockFile {
    ...
    foreach ($allowed in $script:AllowedComponents) {
        if ($File.Name -like "*$allowed*") {
            Write-Log "File ALLOWED (Component Search preserved): $($File.Name)" -Level DEBUG
...
function Invoke-ProcessDirectory {
    ...
    $files = Get-AltiumFiles -Path $Path
    
    if ($files.Count -eq 0) {
        Write-Host "  [WARNING] No Altium files found in this directory" -ForegroundColor Yellow
        return 0
    }
    ...
    for ($i = 0; $i -lt $fileArray.Count; $i++) {
        ...
        Write-Progress -Activity "Analyzing Altium Files" -Status "$percentage% Complete" -PercentComplete $percentage -CurrentOperation $file.Name
        ...
        if ($shouldBlock) {
            ...
            # ALLOW this file (Component Search preserved)
            $allowedCount++
            if ($script:Config.DryRun) {
    ...
    Write-Progress -Activity "Analyzing Altium Files" -Completed
...
function Invoke-ScanSystemLocations {
    ...
    foreach ($basePath in $baseAltiumPaths) {
        if (Test-Path $basePath) {
            # Find all AD* versions (AD20, AD21, AD22, AD23, AD24, AD25, etc.)
            $altiumVersions = Get-ChildItem -Path $basePath -Directory -Filter "AD*" -ErrorAction SilentlyContinue
            foreach ($version in $altiumVersions) {
                $locations += $version.FullName
            }
        }
    }
    
    # Add common Altium locations
    $locations += @(
        "C:\ProgramData\Altium",
        "$env:LOCALAPPDATA\Altium",
        "$env:APPDATA\Altium"
    )
    ...
    if ($locations.Count -eq 0) {
        Write-Host ""
        Write-Host "  [WARNING] No Altium Designer installations found in common locations" -ForegroundColor Yellow
        Write-Host "  [INFO] Would you like to specify a custom Altium directory? (yes/no): " -ForegroundColor Cyan -NoNewline
        $response = Read-Host
        
        if ($response -match '^(yes|y)$') {
            Write-Host "  Enter custom Altium directory path: " -ForegroundColor Cyan -NoNewline
...
function Block-AltiumDomains {
    Write-Host ""
    Write-Host "[Step 5] Blocking Altium domains (SELECTIVE - Component Search preserved)..." -ForegroundColor Cyan
    
    $hostsPath = "$env:SystemRoot\System32\drivers\etc\hosts"
    $hostsBackup = "$hostsPath.backup_$($script:Config.SessionID)"
    
    # ONLY block telemetry and license domains
    # DO NOT block component databases, login servers, or API endpoints
    $domainsToBlock = @(
        # License & Activation (BLOCK)
        "license.altium.com",
        "licensing.altium.com",
        "activate.altium.com",
        "activation.altium.com",
        ...
        "update.altium.com",
        "updates.altium.com",
        "autoupdate.altium.com"
    )
    
    Write-Host "  [INFO] Total domains in block list: $($domainsToBlock.Count)" -ForegroundColor Yellow
    Write-Host "  [INFO] Component databases and login servers are PRESERVED" -ForegroundColor Green
    ...
    try {
        ...
        if ($newEntries.Count -gt 0) {
            $newEntries = @("", "# Altium Designer Blocker Entries (Selective) - Added $(Get-Date)") + $newEntries
...
function Get-AltiumServices {
    ...
    try {
        $services = Get-Service | Where-Object { $_.DisplayName -like "*Altium*" }
        
        if ($services) {
            Write-Host "  [INFO] Found $($services.Count) Altium-related services:" -ForegroundColor Yellow
            ...
            Write-Host "  [INFO] No Altium services detected" -ForegroundColor Cyan
            Write-Log "No Altium services found" -Level INFO
        }
...
function New-ExecutionReport {
    ...
    $report = @"
================================================================================
                ALTIUM DESIGNER BLOCKER EXECUTION REPORT
================================================================================
...
try {
    ...
    switch ($mode) {
        "1" {
            ...
            Write-Host "================================================================================" -ForegroundColor Green
            Write-Host ""
            Write-Host "  Altium Designer telemetry/license blocked successfully!" -ForegroundColor Green
            Write-Host "  Component Search and Login functionality PRESERVED!" -ForegroundColor Green
            Write-Host ""
